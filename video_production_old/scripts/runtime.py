...
class Task(object):
    ...
    def __init__(self, task_id, task_type):
        ...
        if self.tasktype == 1:
            self.startprocess = self._video_info  
        elif self.tasktype == 2:
            self.startprocess = self._video_task
        else:
            self.startprocess = self._video_pic
    ...
    def _create_pic_data(self):
        self.pic = {
            "queuefile":self.tmpath+ self.taskid+ ".video",
            "tmppath":self.encode_tmp_path,
            "logpath":self.encode_tmp_path,
            "outpath":self.encode_tmp_path,
            "vid":TASKID,
            "task_id":TASKID,
            "version_id":'3',
            "enc_log":"%s%s_pic_enc.log"%(LOG_DIR, TASKID),
            "flow_log":"%s%s_pic_flow.log"%(LOG_DIR, TASKID),
            "pic_time":self.obj.cuttimes.replace(",",";"),
            "pic_format":self.obj.pictureformat,
            "dimension":self.obj.dimensions,
            "srtfile":"",
            "logofile":"",
        }
        self.rc.log(self.pic)
...
if __name__ == "__main__":
    ...
    try:
        ...
        print e
        sys.exit()
    sys.path.append('/home/vagrant/workspace/video_production/')
